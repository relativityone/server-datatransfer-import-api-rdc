...
#Region " Error File Management "
		...
		Private Sub LogErrorLine(ByVal values As String())
			...
			If _errorLogFileName = "" Then
				_errorLogFileName = TempFileBuilder.GetTempFile(TempFileBuilder.ErrorsFileNamePrefix)
...
#Region " IArtifactReader Implementation "
		...
		Public Function ManageErrorRecords(ByVal errorMessageFileLocation As String, ByVal prePushErrorLineNumbersFileName As String) As String Implements IArtifactReader.ManageErrorRecords
			...
			If prePushErrorLineNumbersFileName = "" Then
				clientErrors = New System.IO.StreamReader(TempFileBuilder.GetTempFile(TempFileBuilder.ErrorsFileNamePrefix), System.Text.Encoding.Default)
			Else
			...
			Dim errorLinesFileLocation As String = TempFileBuilder.GetTempFile(TempFileBuilder.ErrorsFileNamePrefix)
		...
		Public Function ReadArtifact() As Api.ArtifactFieldCollection Implements Api.IArtifactReader.ReadArtifact
			...
			If _settings.LoadNativeFiles AndAlso Not _settings.NativeFilePathColumn Is Nothing AndAlso Not _settings.NativeFilePathColumn = String.Empty AndAlso collection.FileField Is Nothing Then
				...
				Dim field As New Api.ArtifactField(New DocumentField("File", -1, Relativity.FieldTypeHelper.FieldType.File, Relativity.FieldCategory.FileInfo, Nothing, Nothing, Nothing, True, EDDS.WebAPI.DocumentManagerBase.ImportBehaviorChoice.LeaveBlankValuesUnchanged, False))
			...
			If _settings.CreateFolderStructure AndAlso Not _settings.FolderStructureContainedInColumn Is Nothing AndAlso Not _settings.FolderStructureContainedInColumn = String.Empty Then
				...
				Dim field As New Api.ArtifactField(New DocumentField("Parent", -2, Relativity.FieldTypeHelper.FieldType.Object, Relativity.FieldCategory.ParentArtifact, Nothing, Nothing, Nothing, True, EDDS.WebAPI.DocumentManagerBase.ImportBehaviorChoice.LeaveBlankValuesUnchanged, False))
