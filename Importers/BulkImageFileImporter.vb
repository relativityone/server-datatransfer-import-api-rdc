...
#Region "Main"
		...
		Public Function RunBulkImport(ByVal overwrite As kCura.EDDS.WebAPI.BulkImportManagerBase.OverwriteType, ByVal useBulk As Boolean) As kCura.EDDS.WebAPI.BulkImportManagerBase.MassImportResults
			...
			While tries > 0
				...
				Catch ex As Exception
					tries -= 1
                    If tries = 0 OrElse ExceptionIsTimeoutRelated(ex) OrElse _continue = False OrElse ex.GetType = GetType(Service.BulkImportManager.BulkImportSqlException) OrElse ex.GetType = GetType(MassImport.InsufficientPermissionsForImportException) Then
                        Throw
                    Else
