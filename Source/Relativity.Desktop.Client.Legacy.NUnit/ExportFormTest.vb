...
Namespace Relativity.Desktop.Client.Legacy.NUnit
	...
	Public Class ExportFormTest
		...
		Public Sub SetUpForTypeOfExport(ByVal type As kCura.WinEDDS.ExportFile.ExportType)
			...
			_form.ExportFile = New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
		...
		Public Sub SetupOneFieldSelected(ByVal type As kCura.WinEDDS.ExportFile.ExportType)
			...
			_form = New ExportForm
			_form.ExportFile = New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
			.AllExportableFields = _queryFieldFactory.GetAllDocumentFields.ToArray(),
			.DataTable = _filtersDataTable,
			.ArtifactAvfLookup = avfLookUp,
			.SelectedViewFields = _queryFieldFactory.GetAllDocumentFields().ToArray(),
			.TypeOfExport = type}
			_form.HandleLoad(Nothing, Nothing, 100, 100)
		...
		Public Async Function LoadExportFile_Production_AllExportableFields_OneFieldSelected() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
			 .AllExportableFields = _queryFieldFactory.GetAllDocumentFields.ToArray(),
			 .SelectedViewFields = New kCura.WinEDDS.ViewFieldInfo() {selectedField},
			 .TypeOfExport = kCura.WinEDDS.ExportFile.ExportType.Production}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_Production_AllExportableFields_TwoFieldSelected() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
			 .AllExportableFields = _queryFieldFactory.GetAllDocumentFields.ToArray(),
			 .SelectedViewFields = New kCura.WinEDDS.ViewFieldInfo() {selectedField1, selectedField2},
			 .TypeOfExport = kCura.WinEDDS.ExportFile.ExportType.Production}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_Production_AllExportableFields_TwoFieldSelected_OneFieldIsNoLongerAvaialble() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
			 ...
			 .SelectedViewFields = New kCura.WinEDDS.ViewFieldInfo() {selectedField1, selectedField2, selectedField3},
			 .TypeOfExport = kCura.WinEDDS.ExportFile.ExportType.Production}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(DoesFieldExistsInListBox(selectedField1, _form._columnSelector.RightSearchableListItems))
		...
		Public Async Function LoadExportFile_ProductionPrecedence_OriginalIsDefault() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document)
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportLocation_Set() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.FolderPath = somePath}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_OverwriteFiles_False() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.Overwrite = False}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_OverwriteFiles_True() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.Overwrite = True}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NumberOfDigits_IsNotEmpty() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.VolumeDigitPadding = numberOfDigits}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_SubdirectoryNumberOfDigits_IsNotEmpty() As Task
			Dim numberOfDigits As Int32 = 350
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.SubdirectoryDigitPadding = numberOfDigits}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_UseRelativePath_IsChecked() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.TypeOfExportedFilePath = kCura.WinEDDS.ExportFile.ExportedFilePathType.Relative}
			Await _form.LoadExportFile(ef)
		...
		<Test> Public Async Function LoadExportFile_UseRelativePath_IsNotChecked() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.TypeOfExportedFilePath = kCura.WinEDDS.ExportFile.ExportedFilePathType.Absolute}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_UseAbsolutePath_IsNotChecked() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.TypeOfExportedFilePath = kCura.WinEDDS.ExportFile.ExportedFilePathType.Prefix}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_UsePrefix_IsChecked_TextIsNotEmpty_PrefixTextEnabled() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
			 .TypeOfExportedFilePath = kCura.WinEDDS.ExportFile.ExportedFilePathType.Prefix,
			 .FilePrefix = "TEST"}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._usePrefix.Checked)
			Assert.IsTrue(_form._prefixText.Enabled)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToCustom_FileCharacterInformation_Enabled() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "txt"}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToConcordance_FileCharacterInformation_Disabled() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "dat"}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToCustom_LoadFileCharactersEnabled_ColumnDilimiterSetToPipe() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "txt", .RecordDelimiter = "|"c}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToCustom_LoadFileCharactersEnabled_QuoteDilimiterSetToPipe() As Task
			Dim expectedChar As Char = "|"c
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "txt", .QuoteDelimiter = "|"c}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToCustom_LoadFileCharactersEnabled_NewLineDilimiterSetToPipe() As Task
			Dim expectedChar As Char = "|"c
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "txt", .NewlineDelimiter = "|"c}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToCustom_LoadFileCharactersEnabled_MultiValueDilimiterSetToPipe() As Task
			Dim expectedChar As Char = "|"c
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "txt", .MultiRecordDelimiter = "|"c}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NativeFileFormatSetToCustom_LoadFileCharactersEnabled_NestedValueDilimiterSetToPipe() As Task
			Dim expectedChar As Char = "|"c
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "txt", .NestedValueDelimiter = "|"c}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_AppendOriginalFileName_Checked_True() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.AppendOriginalFileName = True}
			Await _form.LoadExportFile(ef)
		...
		<Ignore("Running longer than 5 seconds")> Public Async Function LoadExportFile_AppendOriginalFileName_Checked_False() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.AppendOriginalFileName = False}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NamedAfter_NotSelected() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportNativesToFileNamedFrom = kCura.WinEDDS.ExportNativeWithFilenameFrom.Select}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NamedAfter_Identifier() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.AppendOriginalFileName = False, .ExportNativesToFileNamedFrom = kCura.WinEDDS.ExportNativeWithFilenameFrom.Identifier}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NamedAfter_Production() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.AppendOriginalFileName = False, .ExportNativesToFileNamedFrom = kCura.WinEDDS.ExportNativeWithFilenameFrom.Production}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_NamedAfter_Custom() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.AppendOriginalFileName = False, .ExportNativesToFileNamedFrom = kCura.WinEDDS.ExportNativeWithFilenameFrom.Custom}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportImages_False() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = False}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportImages_True() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportImages_False_CopyFilesFromRepository_True_All_InTheGroup_Disabled() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = False, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
		...
		Public Async Function LoadExportFile_ExportImages_True_CopyFilesFromRepository_False_All_InTheGroup_Disabled() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = False, .CopyNativeFilesFromRepository = False}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_ImageLoadFileType_Unselected() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .LogFileFormat = Nothing, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_ImageLoadFileType_IPRO() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .LogFileFormat = kCura.WinEDDS.LoadFileType.FileFormat.IPRO, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_ImageLoadFileType_IPRO_FullText() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .LogFileFormat = kCura.WinEDDS.LoadFileType.FileFormat.IPRO_FullText, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_ImageLoadFileType_Opticon() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .LogFileFormat = kCura.WinEDDS.LoadFileType.FileFormat.Opticon, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_TypeOfImage_Unselected() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .TypeOfImage = Nothing, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_TypeOfImage_SinglePage() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .TypeOfImage = kCura.WinEDDS.ExportFile.ImageType.SinglePage, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_TypeOfImage_MultiPageTiff() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .TypeOfImage = kCura.WinEDDS.ExportFile.ImageType.MultiPageTiff, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportImages_True_TypeOfImage_Pdf() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportImages = True, .TypeOfImage = kCura.WinEDDS.ExportFile.ImageType.Pdf, .VolumeInfo = New kCura.WinEDDS.Exporters.VolumeInfo() With
			{.CopyImageFilesFromRepository = True, .CopyNativeFilesFromRepository = True}}
			Await _form.LoadExportFile(ef)
			Assert.IsTrue(_form._exportImages.Checked)
			Assert.IsTrue(_form._imageFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportNatives_False() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportNative = False}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportNatives_False_RightListBoxItemsIsZero() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportNative = False, .SelectedViewFields = New kCura.WinEDDS.ViewFieldInfo() {}}
			Await _form.LoadExportFile(ef)
			Assert.IsFalse(_form._exportNativeFiles.Checked)
			Assert.IsTrue(_form._useAbsolutePaths.Enabled)
			Assert.IsFalse(_form._nativeFileFormat.Enabled)
		...
		Public Async Function LoadExportFile_ExportNatives_True() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportNative = True}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_DataFileFormat_CommaSeparatedCsv() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileExtension = "csv"}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_DataFileFormat_IsHtml() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileIsHtml = True}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_DataFileFormat_IsHtml_AlsoHasLoadFileExtensionField() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileIsHtml = True, .LoadFileExtension = "txt"}
			Await _form.LoadExportFile(ef)
			Assert.AreEqual("HTML (.html)", _form._nativeFileFormat.SelectedItem.ToString)
		End Function
		...
		Public Async Function LoadExportFile_DataFileEncoding_UTF8() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.LoadFileEncoding = System.Text.Encoding.UTF8}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportTextAsFiles_Checked_True() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportFullTextAsFile = True}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportTextAsFiles_Checked_False() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.ExportFullTextAsFile = False}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_TextFileEncoding_ASCII() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.TextFileEncoding = System.Text.Encoding.ASCII}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_TextField_FieldSelected() As Task
			...
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {
			.AllExportableFields = _queryFieldFactory.GetAllDocumentFields.ToArray(),
			.SelectedViewFields = New kCura.WinEDDS.ViewFieldInfo() {textField, boolField},
		...
		Public Async Function LoadExportFile_TextField_BeforeLoad_TwoPotential_AfterLoad_RenamedOnePotential_FirstSelected() As Task
			...
			Dim boolField As kCura.WinEDDS.ViewFieldInfo = _queryFieldFactory.GetAllDocumentFields(2)
			Dim allAvailableFields As New Collections.Generic.List(Of kCura.WinEDDS.ViewFieldInfo)()
			allAvailableFields.AddRange(_queryFieldFactory.GetAllDocumentFields)
			allAvailableFields.Add(longTextFieldRenamed)

			Dim exportAvailable As New Collections.Generic.List(Of kCura.WinEDDS.ViewFieldInfo)()
			...
			_form._textFieldPrecedencePicker.SelectedFields = New List(Of kCura.WinEDDS.ViewFieldInfo)()
		...
		Public Async Function LoadExportFile_ExportMultiChoiceFieldsAsNested_Checked() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.MulticodesAsNested = True}
			Await _form.LoadExportFile(ef)
		...
		Public Async Function LoadExportFile_ExportMultiChoiceFieldsAsNested_NotChecked() As Task
			Dim ef As New kCura.WinEDDS.ExportFile(ArtifactType.Document) With {.MulticodesAsNested = False}
			Await _form.LoadExportFile(ef)
