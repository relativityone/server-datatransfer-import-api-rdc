...
namespace kCura.WinEDDS.Core.NUnit.Export.VolumeManagerV2.Statistics
{
	public class ImageFilesProgressHandlerTests : ProgressHandlerTests
	{
		...
		{
			downloadProgressManager.Verify(x => x.MarkImageAsDownloaded(id, lineNumber), Times.Once);
		}
		...
		{
			downloadProgressManager.Verify(x => x.MarkImageAsDownloaded(id, lineNumber), Times.Never);
		}
	}
}