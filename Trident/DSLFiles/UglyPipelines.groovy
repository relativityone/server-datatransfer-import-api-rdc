...
multibranchPipelineJob('Import-API/Compatibility-pipeline') {
	...
	configure {
		...
		traits << 'jenkins.scm.impl.trait.WildcardSCMHeadFilterTrait' {
			includes('Trident master release-* pipeline-test-*') // branches that you want to include in your job
...
multibranchPipelineJob('Import-API/loadtest-pipeline') {
	...
	configure {
		...
		// IMPORTANT!!!
		traits << 'jenkins.scm.impl.trait.WildcardSCMHeadFilterTrait' {
			includes('Performance-pipeline master pipeline-test-*') // branches that you want to include in your job
			excludes('') // branches you want to exclude from your job
		}
		// you shouldn't need to modify anything under here unless you have some extraordinary use case
		traits << 'com.cloudbees.jenkins.plugins.bitbucket.BranchDiscoveryTrait' {
...
multibranchPipelineJob('Import-API/regression-pipeline') {
	...
	configure {
		...
		// IMPORTANT!!!
		traits << 'jenkins.scm.impl.trait.WildcardSCMHeadFilterTrait' {
			includes('master release-* pipeline-test-*') // branches that you want to include in your job
			excludes('') // branches you want to exclude from your job
		}
		// you shouldn't need to modify anything under here unless you have some extraordinary use case
		traits << 'com.cloudbees.jenkins.plugins.bitbucket.BranchDiscoveryTrait' {
...
multibranchPipelineJob('Import-API/MassImportVerification-pipeline') {
	...
	description('Integration tests for different settings against newest Relativity version from master')
	branchSources {
		branchSource {
			source {
				bitbucket {
	...
	configure {
		...
		// IMPORTANT!!!
		traits << 'jenkins.scm.impl.trait.WildcardSCMHeadFilterTrait' {
			includes('master pipeline-test-*') // branches that you want to include in your job
			excludes('') // branches you want to exclude from your job
		}
		// you shouldn't need to modify anything under here unless you have some extraordinary use case
		traits << 'com.cloudbees.jenkins.plugins.bitbucket.BranchDiscoveryTrait' {
